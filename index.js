		...
		if (message.content.startsWith('!highroll ')) {
			...
			if (userData.data[0] == null) {
				...
				if (Number.isNaN(bet)) {
					...
					var houseString = '';
					var playerString = '';
					const diceFaces = ["0", "<:diceone:1166172997338542150>", "<:dicetwo:1166173045489152084>",
						"<:dicethree:1166173084051574844>", "<:dicefour:1166173115785695352>",
						"<:dicefive:1166173145514913933>", "<:dicesix:1166173180008865842>"];
					const diceRoll = "<a:diceroll:1166173214075002911>";
					var replyMessage = ""
					...
					await wait(2000);
					await msg.edit(houseString)

					const msg1 = await message.reply({
						content: diceRoll + diceRoll + diceRoll + diceRoll + diceRoll
					});
					await wait(2000);
					await msg1.edit(playerString)
			...
			if (userData.data[0] == null) {
				...
				if (Number.isNaN(bet)) {
					...
					if (playerScore > houseScore) {
						...
						const update = { points: userData.data[0].points + bet * mult, streak: userData.data[0].streak + 1, lifetime: userData.data[0].lifetime + bet }
						await axios.put('http://localhost:3000/api/update/' + message.author.id, update)
						replyMessage = "You won!\n\nGambaCoins Won: " + bet + "\nNew GambaCoin Total: " + update.points
					}
